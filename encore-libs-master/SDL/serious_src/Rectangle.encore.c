...
static void _enc__dispatch_Rectangle(pony_ctx_t** _ctx, pony_actor_t* _a, pony_msg_t* _m)
{
  /* Stub! Might be used when we have dynamic dispatch on passive classes */
}


pony_type_t _enc__passive_Rectangle_type = {.id=_ENC__ID_Rectangle, .size=sizeof(_enc__passive_Rectangle_t), .trace=_enc__trace_Rectangle, .dispatch=_enc__dispatch_Rectangle, .vtable=trait_method_selector};
