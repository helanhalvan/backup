...
namespace Ped{
  ...
  private:
    ...
    // Moves all agent towards its next position
    void move();

    // Moves an agent towards its next position
    void calculateNextPosition(Ped::Tagent *agent);
    ...
    std::vector<Ped::Ttree*> *work_load_tmp;

    static bool compare(std::pair<Ped::Ttree*,int> one, std::pair<Ped::Ttree*,int> two);
    void loadBalancing(int num_threads, std::vector<Ped::Ttree*> *work_load);

    int tick_num;
    double lb_time_used = 0;
  };
}
#endif
