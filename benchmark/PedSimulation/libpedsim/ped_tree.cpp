...
void Ped::Ttree::addAgent(const Ped::Tagent *a) {
  if (isleaf) {
    ...
    Ped::Ttree* t = getChildByPosition(a->getX(), a->getY());
    if(t != NULL) {
      t->addAgent(a);
    }
    else {
      cout << "getChildByPosition returned NULL in addAgent #1!!" << endl;
    }
  }
  ...
  if (agents.size() > 8 && depth < maxDepth) {
    ...
    while (!agents.empty()) {
      ...
      if(t != NULL) {
        ...
        cout << "getChildByPosition returned NULL in addAgent #2!!" << endl;
      }
      agents.erase(a);
    }
  }
...
void Ped::Ttree::moveAgent(const Ped::Tagent *a) {
  ...
  if(getChildByPosition(a->getX(), a->getY()) == NULL) {
    agents.erase(a);
    root->addAgent(a);
  }
}
