...
void _enc__trace_Agent_struct(pony_ctx_t* _ctx_arg, void* p)
{
  ...
  encore_trace_object((*_ctx), _enc__field_ss, array_trace);
}
...
array_t* _enc__method_Agent_struct_get_data(pony_ctx_t** _ctx, _enc__passive_Agent_struct_t* _this)
{
  ...
  /* sss = this.ss */;
  array_t* _fieldacc_0 = (*_this)._enc__field_ss;
  array_t* _sss_1 = _fieldacc_0;
  /* this.ss = new [Agent](0) */;
  int64_t _literal_3 = 0;
  array_t* _array_2 = array_mk(_ctx, _literal_3, (&(_enc__passive_Agent_type)));
  (*({ _this;}))._enc__field_ss = _array_2;
  /* sss */;
  return ((array_t*) _sss_1);
}


pony_type_t _enc__passive_Agent_struct_type = {.id=_ENC__ID_Agent_struct, .size=sizeof(_enc__passive_Agent_struct_t), .trace=_enc__trace_Agent_struct, .vtable=trait_method_selector};
