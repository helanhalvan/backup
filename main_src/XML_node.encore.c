...
void _enc__trace_XML_node(pony_ctx_t* _ctx_arg, void* p)
{
  ...
  pony_traceobject((*_ctx), _enc__field_s_rep, _enc__trace_String);
  array_t* _enc__field_children = _this->_enc__field_children;
  pony_traceobject((*_ctx), _enc__field_children, array_trace);
  array_t* _enc__field_comments = _this->_enc__field_comments;
  pony_traceobject((*_ctx), _enc__field_comments, array_trace);
  _enc__passive_String_t* _enc__field_name = _this->_enc__field_name;
  pony_traceobject((*_ctx), _enc__field_name, _enc__trace_String);
  array_t* _enc__field_atribs = _this->_enc__field_atribs;
  pony_traceobject((*_ctx), _enc__field_atribs, array_trace);
}
...
static void _enc__dispatch_XML_node(pony_ctx_t** _ctx, pony_actor_t* _a, pony_msg_t* _m)
{
  /* Stub! Might be used when we have dynamic dispatch on passive classes */
}


pony_type_t _enc__passive_XML_node_type = {.id=_ENC__ID_XML_node, .size=sizeof(_enc__passive_XML_node_t), .trace=_enc__trace_XML_node, .dispatch=_enc__dispatch_XML_node, .vtable=trait_method_selector};
